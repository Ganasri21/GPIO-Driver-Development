...
/*
 * ARM Cortex Mx Procesor NVIC ICERx Register Addresses
 */
...
/*
 * ARM Cortex Mx Procesor Priority Register Addresses
 */
...
#define SPI4_BASEADDR       (APB2PERIPH_BASEADDR + 0x3400)  // 0x4001 3400
...
typedef struct
{
	...
	__vo uint32_t EXTICR[4];     //  External interrupt configuration register - EXTICR[0] - EXTICR1 ... EXTICR[3] - EXTICR4
	uint32_t      RESERVED1[2];
	__vo uint32_t CMPCR;         //  Cell Control register
...
/*
 * IRQ(Interrupt Request) Numbers of STM32F407x MCU  - from Vector Table
 */
 ...
 */
#define SPI1_REG_RESET()      do{ (RCC->APB2ENR |= (1 << 12)); (RCC->APB2RSTR &= ~(1 << 12)); }while(0)
#define SPI2_REG_RESET()      do{ (RCC->APB1ENR |= (1 << 14)); (RCC->APB1RSTR &= ~(1 << 14)); }while(0)
#define SPI3_REG_RESET()      do{ (RCC->APB1ENR |= (1 << 15)); (RCC->APB1RSTR &= ~(1 << 15)); }while(0)
#define SPI4_REG_RESET()      do{ (RCC->APB2ENR |= (1 << 13)); (RCC->APB2RSTR &= ~(1 << 13)); }while(0)
...
#define I2C2_REG_RESET()      do{ (RCC->APB1ENR |= (1 << 22)); (RCC->APB1RSTR &= ~(1 << 22)); }while(0)
#define I2C3_REG_RESET()      do{ (RCC->APB1ENR |= (1 << 23)); (RCC->APB1RSTR &= ~(1 << 23)); }while(0)
